  ...
  if (!CoCoA::handlersEnabled)
  {
    while (true)
    {
      ...
      for (const Poly& g : genericGenerators)
      {
        ...
        if (CoCoA::IsConstant(reduced) && !CoCoA::IsZero(reduced))
        {
          // A nonzero field constant generates the whole polynomial ring.
          d_remainderIsWholeRing = true;
          return;
        }
...
Poly PowerDifferenceIdealMembership::reduce(const Poly& p) const
{
  if (d_remainderIsWholeRing)
  {
    return CoCoA::zero(CoCoA::owner(p));
  }
  Poly reduced = hasPowerDifferenceRules() ? reducePowerRules(p) : p;
...
{
  const bool hasDisequality =
      std::any_of(facts.begin(), facts.end(), [](const Node& fact) {
        return fact.getKind() == Kind::NOT;
      });
  const bool hasPower =
      std::any_of(facts.begin(), facts.end(), [](const Node& fact) {
        return expr::hasSubtermKind(Kind::FINITE_FIELD_POW, fact);
      });
  if (!hasDisequality || !hasPower)
  {
    return std::nullopt;
  }
  ...
  if (!membership.hasPowerDifferenceRules())
  {
    return std::nullopt;
  }
