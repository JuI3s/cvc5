...
TEST_F(TestTheoryFfSplitGb, PowerDifferenceBn254SubfieldMembership)
{
  // BN254 base-field prime q.
  const CoCoA::BigInt q = CoCoA::BigIntFromString(
      "2188824287183927522224640574525727508869631115729782366268903789"
      "4645226208583");
  CoCoA::ring coeffRing = CoCoA::NewZZmod(q);
  std::vector<CoCoA::symbol> syms = CoCoA::symbols("x");
  CoCoA::PPMonoid ppm =
      CoCoA::NewPPMonoidEv(syms, CoCoA::StdDegRevLex, CoCoA::PPExpSize::big);
  CoCoA::SparsePolyRing polyRing = CoCoA::NewPolyRing(coeffRing, ppm);
  CoCoA::RingElem x = CoCoA::indet(polyRing, 0);
  auto xPower = [&](const CoCoA::BigInt& exponent) {
    return CoCoA::monomial(
        polyRing,
        CoCoA::PPMonoidElem(ppm, std::vector<CoCoA::BigInt>{exponent}));
  };
